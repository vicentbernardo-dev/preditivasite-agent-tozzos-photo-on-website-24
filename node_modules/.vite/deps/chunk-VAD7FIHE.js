...
function isGlobalDocumentReference(reference) {
  return !isObject(reference) || typeof reference._ref != "string" ? false : reference._ref.split(":").length === 3;
}
...
var MEDIA_LIBRARY_ASSET_ASPECT_TYPE_NAME = "sanity.asset.aspect";
function isAssetAspect(maybeAssetAspect) {
  return typeof maybeAssetAspect == "object" && !!maybeAssetAspect && "_type" in maybeAssetAspect && maybeAssetAspect._type === "sanity.asset.aspect";
}
function defineAssetAspect(definition) {
  let { assetType, name } = definition;
  return {
    _type: MEDIA_LIBRARY_ASSET_ASPECT_TYPE_NAME,
    _id: `${name}`,
    definition,
    ...assetType && { assetType: Array.isArray(assetType) ? assetType : [assetType] },
...
function isPortableTextTextBlock(value) {
  return isRecord$1(value) && typeof value._type == "string" && Array.isArray(value.children) && value.children.every((child) => isRecord$1(child)) && ("markDefs" in value ? Array.isArray(value.markDefs) && value.markDefs.every((def) => isRecord$1(def)) : true) && ("style" in value ? typeof value.style == "string" : true);
}
...
function isDocumentSchemaType(type) {
  if (!isObjectSchemaType(type)) return false;
  let current = type;
  for (; current; ) {
    if (current.name === "document") return true;
    current = current.type;
  }
  return false;
}
function isObjectSchemaType(type) {
  return isRecord(type) ? type.jsonType === "object" : false;
}
function isArraySchemaType(type) {
  return isRecord(type) ? type.jsonType === "array" : false;
}
...
function isBooleanSchemaType(type) {
  return isRecord(type) ? type.jsonType === "boolean" : false;
}
function isStringSchemaType(type) {
  return isRecord(type) ? type.jsonType === "string" : false;
}
function isDateTimeSchemaType(type) {
  return isStringSchemaType(type) ? type.name === "datetime" : false;
}
function isNumberSchemaType(type) {
  return isRecord(type) ? type.jsonType === "number" : false;
}
...
function isDeprecatedSchemaType(type) {
  return isRecord(type) ? type.deprecated !== void 0 : false;
}
...
function isSpanSchemaType(type) {
  return isRecord(type) ? Array.isArray(type.annotations) && Array.isArray(type.decorators) : false;
}
function isBlockSchemaType(type) {
  if (!isRecord(type) || !Array.isArray(type.fields)) return false;
...
function isBlockStyleObjectField(field) {
  return !isRecord(field) || field.name !== "style" ? false : isRecord(field.type) && field.type.jsonType === "string";
}
function isBlockListObjectField(field) {
  return !isRecord(field) || field.name !== "listItem" ? false : isRecord(field.type) && field.type.jsonType === "string";
}
function isBlockChildrenObjectField(field) {
  return !isRecord(field) || field.name !== "children" || !isArraySchemaType(field.type) ? false : field.type.of.some(isSpanSchemaType);
}
...
//# sourceMappingURL=chunk-VAD7FIHE.js.map
