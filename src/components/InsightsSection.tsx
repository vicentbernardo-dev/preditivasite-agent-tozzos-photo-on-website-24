...
import { blogPostPath, PATHS } from '../routes';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {INSIGHTS.map((article, index) => (
            <Link key={article.id} to={blogPostPath(article.id)} className="block">
              <motion.article
                initial={{ opacity: 0, y: 25 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: index * 0.15 }}
                className="relative flex flex-col justify-between h-full rounded-xl bg-[#111815] border border-white/10 overflow-hidden group hover:border-[#0DF205]/60 transition-all duration-300 cursor-pointer shadow-xl"
              >
                <div>
